...
class parktelModelDetailSpider(Spider):
    name = "parktelmdetailspider"
    ...
    def parse(self,response):
        ...
        price = 0
        priceSelector = response.xpath('//span[contains(@class,"regular-price")]/span[contains(@class,"price")]/text()')
        if len(priceSelector)>0:
            tmpprice = priceSelector[0].extract().strip()
            pricen = self.rc.search(tmpprice)
            if pricen:
                price = float(pricen.group(0).replace(',', ''))

        spepricesel = response.xpath('//span[contains(@class,"special-price")]/span[contains(@class,"price")]/text()')
        if len(spepricesel)>0:
            tmpprice = spepricesel[0].extract().strip()
            pricen = self.rc.search(tmpprice)
            if pricen:
                price = float(pricen.group(0).replace(',', ''))
        ...
        dfeaturemap = {
            "featureTag":[["SIM"],["OS"],
                          ["CPU"],
                          ["Size"],
                          ["Chipset"],["Internal"],["Internal"],
                          ["Card","slot"],["Size"],["Primary"],
        ...
        if "Resolution" in info:
            ...
            resoRe = re.search("(\d+)\s*x\s*(\d+)",screenResStr)
            if resoRe:
